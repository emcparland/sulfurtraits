...
for (p in 1:2){
  ...
  for (genome in 1:nrow(genomessums)){
    ...
    result1 <- result %>% filter(V2 %in% genomescaffolds$V1)
    ...
    for (KO in 1:length(sulfateKO$KO)){
      ...
      if (sulfateKO$KO[KO] %in% result1$V3){
        df[genome,KO]<-1
      } else {
        df[genome,KO]<-0
      }
  ...
  # s_df <- gather( s_df[,1:(ncol(s_dfsteps)-1)],  key = "key", value = "value",Group)
  # plot <- ggplot(s_df, aes(x=factor(key, level=unique(s_df$key)), y = Group, fill = value)) + 
  ...
  #        y = "Group")
  # ggsave(filename = paste0("DOSTraits/kofam_results/", unique(sulfateKOs$Broad.pathway.description[which(sulfateKOs$Pathway.Number == p)]), "_StepsHeatmap.pdf"), plot = plot, width = 20, height = 20, limitsize = FALSE)
  # 
  # 
}
...
# limit the completion of the genomes
