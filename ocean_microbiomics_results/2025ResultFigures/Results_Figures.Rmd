...
```
We will probably only use the parametric tests, which are the plots in this chunk. Robust tests and supplemental figures are later.
```{r}
...
```
Let's make a new panel A without any incompletes.
```{r}
data <- read.csv("20250905_protein.filt.sum.csv", row.names = 1)
our_genomes <- read.csv("new_filtered_SA/2024_June19_ERIN_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
our_genomes <- our_genomes %>% filter(Genome %in% names_id_filtered$Genome)
...
## Panel B
The last plot in this set of plots is the one that has all taxa, with free scale, that we would want to use in supplemental and in the main paper when subsetting the taxa to ones we want to talk about in the paper.
```{r}
s_content_df$taxa_elm <- as.factor(s_content_df$taxa_elm)
s_content_df$Lifestyle <- as.factor(s_content_df$Lifestyle)
df1 <- s_content_df %>% 
...
s_content_df %>% 
  ...
  geom_histogram(alpha=0.6, position = 'identity') + facet_wrap(taxa_elm~., ncol = 5, scales = "free_y") + scale_fill_manual(values = values3)  + theme_minimal()
ggsave(plot = last_plot(), file = paste0("2025Results/newProteinFilter_09052025/fig3b_v2.pdf"), unit = "in", height = 12, width = 10) 
s_content_df %>% 
  ggplot(aes(x = avgS, fill = Lifestyle)) + 
  geom_histogram(alpha=0.6, position = 'identity') + facet_wrap(taxa_elm~., ncol = 1, scales = "free_y") + scale_fill_manual(values = values3)  + theme_minimal()
...
Going to produce a figure for all of the taxa without incompletes, and then select a couple taxa for fun
```{r}
...
ggsave(plot = last_plot(), file = "2025Results/Fig3Variations/fig3_v2.pdf", unit = "in", height = 6, width = 14) 
```
