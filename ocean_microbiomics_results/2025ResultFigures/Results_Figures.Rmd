...
ggsave(filename = "2025Results/fig2.pdf", plot = p3, width=14, height=5)
...
plt + labs(
  x = "Lifestyle", 
  title = "Distribution of S Proteome Content across S Lifestyles"
) + scale_colour_manual(values = c(values3[1], values3[2]))
ggsave(plot = last_plot(), file = "newProteinFilter_09052025/fig3a_2_noincompletes.pdf", unit = "in", height = 6, width = 6) 
...
s_content_df %>% filter(taxa_elm %in% fig3_order3) %>% filter(Lifestyle != "incomplete") %>%
  ggplot(aes(x = avgS, fill = Lifestyle)) + 
  geom_histogram(alpha=0.6, position = 'identity') + facet_wrap(taxa_elm~., ncol = 2, scales = "free_y") + scale_fill_manual(values = c("#0016a6", "#629e02")) + theme_light()
ggsave(plot = last_plot(), file = paste0("newProteinFilter_09052025/fig3b_v2_noincompletes_selecttaxa.pdf"), unit = "in", height = 5, width = 6) 
```
...
## Panel B
This takes the max completion values for each pathway for each genome and makes bar plots out of those values.
```{r}
