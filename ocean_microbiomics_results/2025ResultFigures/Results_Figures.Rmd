...
tcols <- c("#b80020","#8b373f","#ac6863","#ff4f60","#d3004b","sienna","#fdb5a0","#cb6300","#e7c087","#f2b707","#cbcd29","#475907","#8ab200","#95d86c","#008014","#44c442","#01e1a1","#00956e","#009083","#0085e8","#0270f9","#aa79ff","#7431a2","#eb8dff","#ffa8f2","#ff7be8","#ff8bd9","#c30099","#7e486b","#962166","#ffa1c0")
df <- read.csv("genome_summary_MAD_3.csv")
names(tcols) <- sort(unique(df$taxa_elm)[1:31])
```
# Genomes
Import all of our files and get the right number of genomes in these files.
```{r}
# remove genomes with no phyla (no GTDB taxonomy), and make sure we are using this for our taxonomy (include alteromondacaeae)
...
# import the S_i Lifestyles and filter to the correct genomes AND the right taxonomic groups
si_lifestyle <- read.csv("new_filtered_SA/2024_June19_ERIN_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
names(si_lifestyle)[1] <- "Genome"
si_lifestyle$taxa_elm <- NULL
# fix the taxa in the si lifestyle file and save as new file 
si_lifestyle <- merge(si_lifestyle, names_id_filtered, by = "Genome")
write.csv(x = si_lifestyle, "new_filtered_SA/2025_1022_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", quote = F)
si_lifestyle %>% group_by(taxa_elm) %>% summarise(n = n())
si_lifestyle <- si_lifestyle[,c(1,21)]
```
Based on some literature, we need to split the actinobacteria into high and low GC, based on the 40% cutoff we used to the low and high GC rhodoobacters.
```{r}
names_ids <- read.csv("genome_summary_ELM_2.csv")
names_ids <- names_ids[-(which(is.na(names_ids$Phyla))),]
#select the overlap to get 15969
names_id_filtered <- names_ids[which(names_ids$Genome %in% maxcompletion$Genome),]
names_id_filtered %>% filter(taxa_elm == "Actinobacteriota") %>% ggplot(aes(x = GC.Content)) + geom_histogram(bins = 40)
names_id_filtered %>% mutate(new_taxa_elm = case_when(taxa_elm == "Actinobacteriota" & GC.Content < .4 ~ "Actinobacteriota-LowGC", TRUE ~ taxa_elm))
new_names_id_filtered <- names_id_filtered %>% mutate(taxa_elm = if_else(taxa_elm == "Actinobacteriota" & GC.Content < .4, "Actinobacteriota-LowGC", taxa_elm))
write.csv(new_names_id_filtered, file = "genome_summary_MAD_3.csv", row.names = F)
```
Reimport the genomes and check as above.
```{r}
# remove genomes with no phyla (no GTDB taxonomy), and make sure we are using this for our taxonomy (include alteromondacaeae and lowGC-actinobacteria)
names_ids <- read.csv("genome_summary_MAD_3.csv")
names_ids <- names_ids[,c(1,56)]
names(names_ids)[1] <- "Genome"
# remove genomes that have no hits in any pathways
maxcompletion <- read.csv("kofam_results/summary_max_table.csv", row.names = 1)
...
nrow(names_id_filtered)
# check some numbers
head(names_id_filtered %>% group_by(taxa_elm) %>% summarize(n= n()), n = 31)
# import the S_i Lifestyles and filter to the correct genomes AND the right taxonomic groups
si_lifestyle <- read.csv("new_filtered_SA/2025_1022_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
names(si_lifestyle)[1] <- "Genome"
si_lifestyle$taxa_elm <- NULL
# fix the taxa in the si lifestyle file and save as new file 
si_lifestyle <- merge(si_lifestyle, names_id_filtered, by = "Genome")
write.csv(x = si_lifestyle, "new_filtered_SA/2025_1107_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", quote = F)
si_lifestyle %>% group_by(taxa_elm) %>% summarise(n = n())
si_lifestyle <- si_lifestyle[,c(1,21)]
```
...
# Figure 2
## Investigating cysH
This is run on the MIT cluster.
```{r}
...
#FALSE
cysH_genes <- cysH_genes[which(names(cysH_genes) %in% our_genomes)]
all(names(cysH_genes) %in% cysh_gene_list$V5)
...
```
Plotting the cysH tree.
```{r}
#version 1 of the tree has a couple reference genomes 
#version 2 of tree has yeast and those reference genomes
#version 2 of tree has yeast and those reference genomes and rhizobium
tree1 <- read.tree("2025Results/cysH_tree2_yeastrooted")
p7 <- ggtree(tree1, layout = "fan", open.angle=15, size =0.1) + geom_tiplab(size = 0)
p8 <- ggtree(tree1, size =0.1) + geom_tiplab(size = 0.5) + xlim(0,6) + ylim(-1,5345) #+ geom_text(aes(label = node))
df5 <- names_id_filtered
df5 <- rbind(df5, c("Mycobacterium_cysH", "Mycobacterium"))
df5 <- rbind(df5, c("Ecoli_cysH", "E.coli"))
df5 <- rbind(df5, c("Burkholderiacepacia_cysH", "Burkholderiacepacia"))
df5 <- rbind(df5, c("Scerevisiae_cysH", "Root"))
df5 <- rbind(df5, c("Thiocapsaroseopersicina_cysH", "Thiocapsaroseopersicina"))
df5 <- rbind(df5, c("Rhizobium_cysH", "Rhizobium"))
row.names(df5) <- df5$Genome
df5 <- select(df5, taxa_elm)
df5$taxa_elm <- as.factor(df5$taxa_elm)
length(get_taxa_name(p7))
get_taxa_name(p7)[5321:5336]
# colors to taxa names
tcols2 <- c(tcols, c("#000000", "#dddddd","#333333", "#ff0000", "#ffffff", "#212121"))
names(tcols2)[32:36] <-c("Mycobacterium", "E.coli", "Burkholderiacepacia","Root","Thiocapsaroseopersicina", "Rhizobium")
tcols2
p9 <- p8 + geom_hilight(node=6412, fill='darkgray', type="rect") + 
  geom_hilight(node=5352, fill='#dddddd', type="rect") +
  geom_cladelab(node=6412, label="Similar to APS reductases from plants", align=TRUE, 
                  geom='label', textcolor='black', angle = 90,hjust='center', offset.text=.1) +
  geom_cladelab(node=5352, label="Similar to PAPS reductase", align=TRUE, 
                  geom='label', textcolor='darkgray', barcolor = "darkgray", hjust='center', offset.text=.1, angle = 90)
gheatmap(p8, df5, offset=0.2, width=0.2, 
               colnames=TRUE,colnames_offset_x = 0, color = NA,
               hjust = 1, legend_title="Taxa", colnames_angle = 90, 
               colnames_offset_y = 0) 
gheatmap(p9, df5, offset=0.2, width=0.2, 
               colnames=TRUE,colnames_offset_x = 0, color = NA,
               hjust = 1, legend_title="Taxa", colnames_angle = 90, 
               colnames_offset_y = 0) + scale_fill_manual(values = tcols2)
ggsave(plot = last_plot(), file = "2025Results/cysH_tree.pdf", unit = "in", height = 40, width = 10, limitsize = FALSE) 

tree1 <- read.tree("2025Results/cysH_tree3_yeastrooted")
df5 <- names_id_filtered
df5 <- rbind(df5, c("Mycobacterium_cysH", "Mycobacterium"))
df5 <- rbind(df5, c("Ecoli_cysH", "E.coli"))
df5 <- rbind(df5, c("Burkholderiacepacia_cysH", "Burkholderiacepacia"))
df5 <- rbind(df5, c("S.cerevisiae_cysH", "Root"))
df5 <- rbind(df5, c("Thiocapsaroseopersicina_cysH", "Thiocapsaroseopersicina"))
df5 <- rbind(df5, c("Rhizobium_cysH", "Rhizobium"))
row.names(df5) <- df5$Genome
df5 <- select(df5, taxa_elm)
...
length(get_taxa_name(p7))
# colors to taxa names
tcols2 <- c(tcols, c("#000000", "#dddddd","#333333", "#ff0000", "#ffffff", "#121212"))
names(tcols2)[32:37] <-c("Mycobacterium", "E.coli", "Burkholderiacepacia","Root","Thiocapsaroseopersicina", "Rhizobium")
tcols2
p8 <- ggtree(tree1, size =0.1) + geom_tiplab(size = 0) + xlim(0,6) + ylim(-1,5345) #+ geom_text(aes(label = node))
p9 <- p8 + geom_hilight(node=6413, fill='darkgray', type="rect") + 
  geom_hilight(node=5353, fill='#dddddd', type="rect") +
  geom_cladelab(node=6413, label="Similar to APS reductases from plants", align=TRUE, 
                  textcolor='black', barcolor='black', angle = 90,hjust='center', offset.text=.1) +
  geom_strip('MARD_SAMN05418229_REFG_MMP05418229', 'S.cerevisiae_cysH', color='darkgray', 
            label = "Similar to PAPS reductase", offset.text=.1, hjust='center',angle = 90)
gheatmap(p9, df5, offset=0.2, width=0.2, 
               colnames=TRUE,colnames_offset_x = 0, color = NA,
               hjust = 1, legend_title="Taxa", colnames_angle = 90, 
               colnames_offset_y = 0) + scale_fill_manual(values = tcols2)
ggsave(plot = last_plot(), file = "2025Results/cysH_tree2.pdf", unit = "in", height = 40, width = 10, limitsize = FALSE) 
# let's get the taxa in node 6413:
apr_plantlike <- as.data.frame(get_taxa_name(p8, node = 6413))
# add them to the sulfate assimilation RAW data we use
results <- read.table("new_filtered_SA/2024_0531_sulf_out_filt.txt")
apr_plantlike <- cbind("K05907", apr_plantlike, NA, NA, NA, NA, NA, "cysH similar to APS reductase from plants")
apr_plantlike <- merge(apr_plantlike, names_id_filtered, by.x = 2, by.y=1)
apr_plantlike$new <- NA
apr_plantlike <- apr_plantlike[,c(2,1,3:10)]
names(apr_plantlike)<- names(results)
results_withcyslikeapr <- rbind(results, apr_plantlike)
write.table(results_withcyslikeapr, file = "new_filtered_SA/2025_1110_sulf_out_filt.txt", sep = "\t")
```
### Rerun sulfate assimilation logic with the new cysH discovery
Using txt file made above
```{r}
results <- read.table("new_filtered_SA/2025_1110_sulf_out_filt.txt")
# get the KOs
sulfateKOs <- read.csv("Sulfate_20251110.csv")
sulfateKOs$StepKOs <- paste0("Step", sulfateKOs$Step, ":", sulfateKOs$KO)
genomessums <- read.csv("genome_summary_ELM_2.csv")
logics <- read.csv("SulfateLogic_20251110.csv",header = 1)
...
for (n in 1:nrow(genomessums)){
  #print(n)
  # filter the hits in the all filtered hits file to just the one genome
  result1 <- result %>% filter(genome == genomessums$Genome[n])
  ...
  row.names(df)[n] <- genomessums$Genome[n]
}
write.csv(x = df, file = paste0("new_filtered_SA/2025_1110_new_filtered_SA_Sulfate_AssimilationRaw.csv"))
df <- df[match(genomessums$Genome, row.names(df)),]
```
### Process the sulfate assimilation raw results
```{r}
p <- 1
sulfateKO <- sulfateKOs %>%
  filter(Pathway.Number == p)
...
all(row.names(dfsteps) == genomessums$Genome)
# if true, then can:
dfsteps$taxa_elm <- genomessums$taxa_elm
dfsteps$Mean.Completeness <- genomessums$Mean.Completeness
dfsteps$taxa_elm <- as.factor(dfsteps$taxa_elm)
...
which(is.na(genomessums$Phyla))
dfsteps <- dfsteps[-which(is.na(genomessums$Phyla)),]
  
#save this file
write.csv(dfsteps, file = paste0("new_filtered_SA/2025_1110_MAD_SulfateAssimilationprocessed.csv"), quote = F)

dfsteps50 <- dfsteps %>%
  filter(Mean.Completeness >= 50)
dfsteps75 <- dfsteps %>%
  filter(Mean.Completeness >= 75)
write.csv(dfsteps50, file = paste0("new_filtered_SA/2025_1110_MAD_SulfateAssimilationprocessed_above50genomecompletion.csv"), quote = F)
...
# check some numbers
head(names_id_filtered %>% group_by(taxa_elm) %>% summarize(n= n()), n = 31)
dfsteps75 <- dfsteps75[which(row.names(dfsteps75) %in% names_id_filtered$Genome),]
dfsteps75$taxa_elm <- NULL
# fix the taxa in the si lifestyle file and save as new file 
dfsteps75 <- merge(dfsteps75, names_id_filtered, by.x = 0, by.y = "Genome")
names(dfsteps75)[1] <- "Genome"
write.csv(dfsteps75, file = paste0("new_filtered_SA/2025_1110_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv"), quote = F)

si_lifestyle <- dfsteps75
si_lifestyle %>% group_by(taxa_elm) %>% summarise(n = n())
si_lifestyle <- si_lifestyle %>% select(taxa_elm, Lifestyle)
```
...
```{r}
#OLD
our_genomes_old <- read.csv("new_filtered_SA/2025_1107_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
# limit to 15969 correct genomes from chunk above
our_genomes_old <- our_genomes_old %>% filter(Genome %in% names_id_filtered$Genome)
#check sums of auxotrophs, assimilators and incompletes
our_genomes_old %>% group_by(Lifestyle) %>% summarise(n = n())
#NEW
our_genomes <- read.csv("new_filtered_SA/2025_1110_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
# limit to 15969 correct genomes from chunk above
our_genomes <- our_genomes %>% filter(Genome %in% names_id_filtered$Genome)
#check sums of auxotrophs, assimilators and incompletes
our_genomes %>% group_by(Lifestyle) %>% summarise(n = n())

# figure out differences between the two and make sure the numbers are correct 
genomeswithplantlikeapr <- df %>% rownames_to_column(var = "genome") %>% filter(genome %in% names_id_filtered$Genome) %>% filter(`Step7:K05907`==1) %>% select(genome)
# these are the genomes that could possibly be affected by the change we made
# let's see which ones already have a 1:
genomeswithplantlikeapr_alreadycounted <- our_genomes_old %>% filter(Lifestyle == "assimilator" & Genome %in% genomeswithplantlikeapr$genome) %>% select(Genome)
#subtract those already counted
genomeswithplantlikeapr_minusalreadycounted <- genomeswithplantlikeapr[-which(genomeswithplantlikeapr$genome %in% genomeswithplantlikeapr_alreadycounted$Genome),]
#see which of these would be able to reach one, so need step 9 and need 1 2 or 3
our_genomes_old %>% filter(Genome %in% genomeswithplantlikeapr_minusalreadycounted) %>% filter(Step9 == 1 & (Step1 == 1 | Step2 == 1 | Step3 ==1)) %>% summarise(numbergenomesturnedfromincompletetoassimilator = n())
...
p2
p3 <- ggarrange(p1, p2, align = "hv", legend = "top", widths = c(8,4))
...
p2 <- ggplot(table1, aes(x = taxa_elm, y = n, fill = mean)) + geom_bar(stat = "identity") +
  ...
  annotate("text", y = 20 +table1$n[which(table1$taxa_elm == order1[11])], x = 11, hjust = 0, size = 2,label = round(table1$mean[which(table1$taxa_elm == order1[11])],1)) +
  annotate("text", y = table1$n[which(table1$taxa_elm == order1[10])] -20, x = 10, hjust = 1, size = 2,color = "white", label = round(table1$mean[which(table1$taxa_elm == order1[10])],1)) +
  ...
  annotate("text", y = 20 +table1$n[which(table1$taxa_elm == order1[31])], x = 31, hjust = 0, size = 2,label = round(table1$mean[which(table1$taxa_elm == order1[31])],1))
p2
p3 <- ggarrange(p1, p2, align = "hv", legend = "top", widths = c(8,4))
ggsave(filename = "2025Results/Fig2Variations/fig2_annotated.pdf", plot = p3, width=14, height=5)

order2 <- c(order1[2], order1[1], order1[3:31])
...
p2 <- ggplot(table1, aes(x = taxa_elm, y = n, fill = mean)) + geom_bar(stat = "identity") +
  ...
  annotate("text", y = 20 +table1$n[which(table1$taxa_elm == order2[11])], x = 11, hjust = 0, size = 2,label = round(table1$mean[which(table1$taxa_elm == order2[11])],1)) +
  annotate("text", y = table1$n[which(table1$taxa_elm == order2[10])] -20, x = 10, hjust = 1, size = 2,color = "white", label = round(table1$mean[which(table1$taxa_elm == order2[10])],1)) +
  ...
  annotate("text", y = 20 +table1$n[which(table1$taxa_elm == order2[31])], x = 31, hjust = 0, size = 2,label = round(table1$mean[which(table1$taxa_elm == order2[31])],1))
 p2
p3 <- ggarrange(p1, p2, align = "hv", legend = "top", widths = c(8,4))
ggsave(filename = "2025Results/Fig2Variations/fig2_differentorder_annotated.pdf", plot = p3, width=12, height=5)
...
our_genomes <- read.csv("new_filtered_SA/2025_1107_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
our_genomes <- our_genomes %>% filter(Genome %in% names_id_filtered$Genome)
s_content_df <- merge(x = our_genomes[,c(1,21,22)], y = data[,-c(2)], by.x = "Genome", by.y = "genome")
...
```{r}
data <- read.csv("20250905_protein.filt.sum.csv", row.names = 1)
our_genomes <- read.csv("new_filtered_SA/2025_1022_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv", row.names = 1)
our_genomes <- our_genomes %>% filter(Genome %in% names_id_filtered$Genome)
s_content_df <- merge(x = our_genomes[,c(1,21,22)], y = data[,-c(2)], by.x = "Genome", by.y = "genome")
s_content_df %>% group_by(Lifestyle) %>% summarise(mean = mean(avgS))
...
s_content_df %>% group_by(taxa_elm, Lifestyle) %>% filter(n() > 10) %>%
  ...
  geom_density(aes(y = ..count.. * 0.0005, fill = Lifestyle, color = Lifestyle) , alpha=0.1) + 
  geom_histogram(aes(y = ..count.., fill = Lifestyle), alpha=0.6, position = 'identity', binwidth = 0.0005) + 
  facet_wrap(taxa_elm~., ncol = 1, scales = "free_y", strip.position="right") + 
  scale_fill_manual(values = c("#0016a6", "#0271a8", "#497502")) + 
  scale_color_manual(values = c("#0016a6", "#0271a8", "#497502")) + 
  theme_minimal() + 
  theme(panel.grid.minor = element_blank(), panel.spacing = unit(0, "lines"), strip.text.y = element_text(angle = 0)) + 
...
# try to use geom density ridges to make the ridges overlap instead of being stumpy
assim_d <- density(s_content_df$avgS[which(s_content_df$Lifestyle == "assimilator")])
aux_d <- density(s_content_df$avgS[which(s_content_df$Lifestyle == "auxotroph")])
incom_d <- density(s_content_df$avgS[which(s_content_df$Lifestyle == "incomplete")])
assim_length <- length(which(s_content_df$Lifestyle == "assimilator"))
aux_length <- length(which(s_content_df$Lifestyle == "auxotroph"))
incom_length <- length(which(s_content_df$Lifestyle == "incomplete"))
dif <- tidyr::pivot_longer(data.frame(s_content = assim_d$x, 
                               assim_1 = assim_d$y * assim_length/incom_length, 
                               incom_1 = incom_d$y, 
                               aux_1 = aux_d$y * aux_length/incom_length, 
                               assim_2 = assim_d$y * assim_length/incom_length, 
                               incom_2 = incom_d$y, 
                               aux_2 = aux_d$y * aux_length/incom_length, 
                               assim_3 = assim_d$y * assim_length/incom_length, 
                               incom_3 = incom_d$y, 
                               aux_3 = aux_d$y * aux_length/incom_length), -1, names_to = "bin")
dif$num <-NA
dif$num[grep("1", dif$bin)] <- 1
dif$num[grep("2", dif$bin)] <- 2
dif$num[grep("3", dif$bin)] <- 3
ggplot(dif, aes(x = s_content, y = num, height = value, fill = bin)) + 
  geom_density_ridges(stat = "identity", alpha = 0.4)+
  labs(x = "S content", y = "lifestyle")+
  theme_minimal()


s_content_df %>% ggplot(aes(x = avgS, fill = Lifestyle)) + geom_histogram(alpha = 0.6)
s_content_df %>% ggplot()
tidyr::pivot_longer(data.frame(yrs = density(df1$yrs)$x,
                      foo1 = density(df1$yrs)$y * nrow(df1)/nrow(df2),
                      foo2 = density(df2$yrs)$y), -1, names_to = "bin")
...
fig3_order3 <- c("Firmicutes", "Alpha-Pelagibacter", "Gamma-Alteromonadaceae", "Alpha-Rhodobacter")
...
plt <- plt + labs(
  x = "Lifestyle", 
  title = "Distribution of S Proteome Content across S Lifestyles"
) + scale_colour_manual(values = c(values3[1], values3[2])) + theme(plot.title = element_text(size = 12, face = "plain", hjust = 0.5), plot.subtitle = element_blank(), axis.title.x = element_text(face = "plain"), axis.title.y = element_text(face = "plain"), plot.caption = element_blank()) + labs(title = "A. Distribution of Sulfur Proteome Content Across Aquisition Strategies") + ylab("Average Sulfur Content of Proteome") + xlab("Lifestyle") + scale_x_discrete(labels = c("Sulfate Assimilator\n(n = 3,189)", "Putative Auxotroph\n(n = 5,339)"))

#bring in the second panel we want
fig3_order3 <- c("Alpha-Pelagibacter", "Gamma-Alteromonadaceae")
plt2 <- s_content_df %>% filter(taxa_elm %in% fig3_order3) %>% filter(Lifestyle != "incomplete") %>%
  ggplot(aes(x = avgS, fill = Lifestyle)) + 
  geom_histogram(alpha=0.6, position = 'identity') + facet_wrap(taxa_elm~., ncol = 2, scales = "free_y") + scale_fill_manual(values = c("#0016a6", "#497502"), labels = c("Sulfate Assimilator", "Putative Auxotroph"), name = "Sulfur Acquisition Strategy") + theme_bw() + theme(plot.title = element_text(size = 12, face = "plain", hjust = 0.5), plot.subtitle = element_blank(), axis.title.x = element_text(face = "plain"), axis.title.y = element_text(face = "plain"), plot.caption = element_blank()) + labs(title = "B. Distribution of Sulfur Proteome Content Across Select Taxa") + xlab("Average Sulfur Content of Proteome") + ylab("Count of Genomes") + geom_vline(data = taxa_s_content_summary[which(taxa_s_content_summary$taxa_elm %in% fig3_order3),], aes(xintercept = mean_s), linetype = "dashed") + theme(legend.position = "bottom") 

#combine
ggarrange(plt, plt2, align = "hv", widths = c(5,7))
ggsave(plot = last_plot(), file = "2025Results/newProteinFilter_09052025/Fig3Variations/fig3_v2.pdf", unit = "in", height = 6, width = 14) 
```

# Figure 4
This figure looks at the pathways per genome, and includes indicator analysis. First, we need to modify some of the pathways manually.

## Panel B
###Checking some numbers first.
...
# let's look at subpathway 6
test_df %>% select(Step10, Step11, Step14, Step7, Step8, Step4, Group) %>% pivot_longer(cols = where(is.numeric)) %>% ggplot(aes(x = name, y = value)) + geom_boxplot() + geom_point(position = "jitter", alpha = 0.1) + facet_wrap(Group~., nrow = 5)
test_df %>% select(Step10, Step11, Step14, Step7, Step8, Step4, Group) %>% mutate(sum = Step10 + Step11 + Step14 + Step7 + Step8 + Step4) %>% filter(sum > 0.8) %>% summarize(n = n())
test_df %>% filter(MaxCompletion < 1 & MaxCompletion > 0.5) %>% select(Step7, Step8, Step4, Group) %>% pivot_longer(cols = where(is.numeric)) %>% ggplot(aes(x = name, y = value)) + geom_boxplot() + geom_point(position = "jitter", alpha = 0.1) + facet_wrap(Group~., nrow = 5)
# being that subpathway 6 is one of the canonical pathways and most of the genomes have 5 or 6 steps (15768 genomes), I think we are safe with the conversion we made earlier :)
```
...
#what do we do here?
# michelle's vote: ~2000 have the first step, so we turn those to complete but otherwise, no.
```

### Modifying the pathway max completion values
We looked at 6 pathways (Cysteine biosynthesis, Cysteine.Degradation, Methionine.Synthesis, Glutathione.Synthesis, Methionine.Degradation, and Choline.O.sulfate.Degradation) and went through to see if we needed to modify a step and remove it to consider a different value as complete. This assessment is in CheckingPathwayNumbersforFigure4.rmd, and is also above, though the above is missing some of the final assessments I made.

```{r}
# Cysteine biosynthesis: keep
# Cysteine.Degradation: keep
# Methionine.Synthesis: 
# just change those above .66 to 1
maxcompletion <- maxcompletion %>% mutate(Methionine.Synthesis = if_else(Methionine.Synthesis > 0.66, 1, Methionine.Synthesis))
# Glutathione.Synthesis: keep
# Methionine.Degradation: remove step 2 from the steps
test_df <- read.csv("kofam_results/Methionine Degradationprocessed_above75genomecompletion.csv", row.names = 1)
test_df <- test_df %>% mutate(sumstep1_2 = Step1 + Step3) 
test_df <- test_df %>% mutate(MaxCompletion = if_else(sumstep1_2 == 2, 1, MaxCompletion))
write.csv(test_df[,1:10], "kofam_results/Methionine Degradationprocessed_above75genomecompletion_20251022.csv", row.names = T)
## add this to the max completion table
select(test_df, "MaxCompletion")
genomes_order <- maxcompletion$Genome
summary(test_df$MaxCompletion[match(genomes_order, row.names(test_df))])
summary(maxcompletion$Methionine.Degradation)
maxcompletion[,which(names(maxcompletion)=="Methionine.Degradation")] <- test_df$MaxCompletion[match(genomes_order, row.names(test_df))]
summary(maxcompletion$Methionine.Degradation)
# Choline.O.sulfate.Degradation: 
test_df <- read.csv("kofam_results/Choline-O-sulfate Degradationprocessed_above75genomecompletion.csv", row.names = 1)
summary(test_df$MaxCompletion)
test_df <- test_df %>% mutate(MaxCompletion = if_else(Step1 == 1, 1, MaxCompletion))
summary(test_df$MaxCompletion)
write.csv(test_df, "kofam_results/Choline-O-sulfate Degradationprocessed_above75genomecompletion_20251022.csv", row.names = T)
## add this to the max completion table
select(test_df, "MaxCompletion")
genomes_order <- maxcompletion$Genome
summary(test_df$MaxCompletion[match(genomes_order, row.names(test_df))])
summary(maxcompletion$Choline.O.sulfate.Degradation)
maxcompletion[,which(names(maxcompletion)=="Choline.O.sulfate.Degradation")] <- test_df$MaxCompletion[match(genomes_order, row.names(test_df))]
summary(maxcompletion$Choline.O.sulfate.Degradation)
# save new max table
write.csv(x = maxcompletion, "kofam_results/summary_max_table_20251024.csv")
```
...
# use the line above or do not if you want the incompletes removed
print("Removing:")
...
p4 <- ggplot(df2, aes(x = factor(key, order), fill = value)) + geom_bar() + 
  ...
  scale_fill_manual(values = values9[c(1,3)]) +
...
p4 <- ggplot(df2, aes(x = factor(key, order), fill = value)) + geom_bar() + 
  theme_linedraw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .)+
  scale_fill_manual(values = values9[c(4,6)]) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) + theme(axis.text.x = element_text(size = 14))
p4
...
p4 <- ggplot(df2, aes(x = factor(key, order), fill = value)) + geom_bar() + 
  theme_linedraw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .)+
  scale_fill_manual(values = values9[c(7,9)]) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) + theme(axis.text.x = element_text(size = 14))
p4
...
core_pathways <- df3$key[which(df3$count >= .75*15969)]
order <- rev(df3$key)
df2$core_value <- NA
...
p5 <- ggplot(df2, aes(x = factor(key, order))) + 
  ...
  xlab("Pathway")  
p5
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberperpathway.pdf", unit = "in", height = 20, width = 20) 
...
p6 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberperlifestyle.pdf", unit = "in", height = 20, width = 20) 
```
...
df2 <- gather(df1, key = "key", value = "value", Cysteine.Biosynthesis:S.adenosyl.L.homocysteine.degradation)
df2$value <- factor(df2$value)
df2 <- df2 %>% mutate(across(4, \(x)if_else(x=="0", "Absent", x))) %>% mutate(across(4, \(x)if_else(x=="1", "Present", x))) %>% mutate(across(4, \(x)if_else(x!="Absent" & x!="Present", "Incomplete", x)))
df2$value <- factor(df2$value)
df2$Lifestyle <- factor(df2$Lifestyle)
df2$key <- factor(df2$key)
df3 <- df2 %>% filter(value == "Present") %>% group_by(key) %>% summarise(count = n()) %>% arrange(., count)
...
p4 <- ggplot(df2, aes(x = factor(key, order), fill = Lifestyle, alpha = value)) + geom_bar() + 
...
df2 <- df2 %>%
  mutate(value_color = if_else(value == "Absent" & Lifestyle == "incomplete", "6", as.factor(value))) %>%
  mutate(value_color = if_else(value == "Absent" & Lifestyle == "auxotroph", "9", as.factor(value_color))) %>%
  mutate(value_color = if_else(value == "Present" & Lifestyle == "incomplete", "4", as.factor(value_color))) %>%
  mutate(value_color = if_else(value == "Present" & Lifestyle == "auxotroph", "7", as.factor(value_color))) %>%
  mutate(value_color = if_else(value == "Absent" & Lifestyle == "assimilator", "3", as.factor(value_color)))%>%
  mutate(value_color = if_else(value == "Present" & Lifestyle == "assimilator", "1", as.factor(value_color))) %>%
  mutate(value_color = if_else(value == "Incomplete" & Lifestyle == "incomplete", "5", as.factor(value_color))) %>%
  mutate(value_color = if_else(value == "Incomplete" & Lifestyle == "auxotroph", "8", as.factor(value_color))) %>%
  mutate(value_color = if_else(value == "Incomplete" & Lifestyle == "assimilator", "2", as.factor(value_color)))
df2 <- df2 %>% mutate(value_color = as.factor(value_color)) 
summary(df2)
df2$Lifestyle <- factor(df2$Lifestyle, levels = c("assimilator", "incomplete", "auxotroph"))
c("1", "2", "3", "4", "5", "6", "7", "8", "9")
...
p4 <- ggplot(df2, aes(x = factor(key, order), fill = value_color)) + geom_bar() + 
  theme_bw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ ., scales = "free_y") +
  scale_fill_manual(values = values9) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) + theme(axis.text.x = element_text(size = 14)) 
p4
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_v3.pdf", unit = "in", height = 20, width = 20) 

p4 <- ggplot(df2, aes(x = factor(key, order), fill = value_color)) + geom_bar(position = "fill") + 
  theme_bw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .) +
  scale_fill_manual(values = values9) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) + theme(axis.text.x = element_text(size = 14)) 
p4
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_v4.pdf", unit = "in", height = 20, width = 20) 

fd_IV <- read.csv("2025Results/IndicatorValues_2.csv", row.names = 1)
df_Indgroup <- read.csv("2025Results/IndicatorGroups_2.csv", row.names = 1)[,1:2][duplicated(read.csv("2025Results/IndicatorGroups_2.csv", row.names = 1)[,1:2]),]
...
common_paths <- unique(fd_IV$ind.vari[which(fd_IV$indicator.group == "common")])
aux_paths <- unique(fd_IV$ind.vari[which(fd_IV$indicator.group == "auxotroph")])
assim_paths <- unique(fd_IV$ind.vari[which(fd_IV$indicator.group == "assimilator")])

df2 %>% filter(Lifestyle != "incomplete") %>% filter(key %in% common_paths) %>% 
  ggplot(aes(x = factor(key, order), fill = value_color)) + 
  geom_bar(position = "fill") + 
  theme_bw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .) +
  scale_fill_manual(values = values9[c(1:3,7:9)]) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) + theme(axis.text.x = element_text(size = 14)) 

ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_commonindicators.pdf", unit = "in", height = 20, width = length(common_paths) + 2) 

df2 %>% filter(Lifestyle != "incomplete") %>% filter(key %in% aux_paths) %>% 
  ggplot(aes(x = factor(key, order), fill = value_color)) + 
  geom_bar(position = "fill") + 
  theme_bw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .) +
...
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_auxindicators.pdf", unit = "in", height = 20, width = length(aux_paths) + 2)

 df2 %>% filter(Lifestyle != "incomplete") %>% filter(key %in% assim_paths) %>% 
  ggplot(aes(x = factor(key, order), fill = value_color)) + 
  geom_bar(position = "fill") + 
  theme_bw() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .) +
...
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_assimilatorindicators.pdf", unit = "in", height = 20, width = length(assim_paths) + 2)

df2 %>% filter(Lifestyle != "incomplete") %>% filter(key %in% common_paths) %>% 
  ggplot(aes(x = factor(key, order), fill = value_color)) + 
  geom_bar(position = "fill") + 
  theme_bw() +theme(axis.text.x = element_blank()) + xlab("Pathway") + 
  facet_grid(Lifestyle ~ .) +
  scale_fill_manual(values = values9[c(1:3,7:9)]) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) + theme(axis.text.x = element_text(size = 14)) 

ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_commonindicators_blank.pdf", unit = "in", height = 20, width = length(common_paths) + 2) 
...
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_auxindicators_blank.pdf", unit = "in", height = 20, width = length(aux_paths) + 2)
...
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_assimilatorindicators_blank.pdf", unit = "in", height = 20, width = length(assim_paths) + 2)

df2 %>% filter(key %in% assim_paths) %>% filter(value == "Present") %>% ggplot(aes(x = factor(key, order))) + 
  geom_bar(fill = "black") + theme_bw() + theme(axis.text.x = element_text( angle = 45, vjust = 1, hjust=1)) + 
  xlab("Pathway") + ylim(0,16000)
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberper_assimind.pdf", unit = "in", height = 5, width = length(assim_paths) + 2) 

df2 %>% filter(key %in% aux_paths) %>% filter(value == "Present")%>% ggplot(aes(x = factor(key, order))) + 
  geom_bar(fill = "black") + theme_bw() + theme(axis.text.x = element_text( angle = 45, vjust = 1, hjust=1)) + 
  xlab("Pathway")  + ylim(0,16000)
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberper_auxind.pdf", unit = "in", height = 5, width = length(aux_paths) + 2) 

df2 %>% filter(key %in% common_paths) %>% filter(value == "Present")%>% ggplot(aes(x = factor(key, order))) + 
  geom_bar(fill = "black") + theme_bw() + theme(axis.text.x = element_text( angle = 45, vjust = 1, hjust=1)) + 
  xlab("Pathway")  + ylim(0,16000)
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberper_commonind.pdf", unit = "in", height = 5, width = length(common_paths) + 2) 

df2 %>% filter(key %in% assim_paths) %>% filter(value == "Present") %>% ggplot(aes(x = factor(key, order))) + 
  geom_bar(fill = "black") + theme_bw() + theme(axis.text.x = element_blank()) + 
  xlab("Pathway")  + ylim(0,16000)
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberper_assimind_blank.pdf", unit = "in", height = 5, width = length(assim_paths) + 2) 

df2 %>% filter(key %in% aux_paths) %>% filter(value == "Present")%>% ggplot(aes(x = factor(key, order))) + 
  geom_bar(fill = "black") + theme_bw() + theme(axis.text.x = element_blank()) + 
  xlab("Pathway")  + ylim(0,16000)
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberper_auxind_blank.pdf", unit = "in", height = 5, width = length(aux_paths) + 2) 

df2 %>% filter(key %in% common_paths) %>% filter(value == "Present")%>% ggplot(aes(x = factor(key, order))) + 
  geom_bar(fill = "black") + theme_bw() + theme(axis.text.x = element_blank()) + 
  xlab("Pathway")  + ylim(0,16000)
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_numberper_commonind_blank.pdf", unit = "in", height = 5, width = length(common_paths) + 2) 

```
### Looking at numbers per pathway for the paper
```{r}
temp <- merge(df1, names_id_filtered, by = "Genome")
# DHPS Degradation - is undifferentiated realtive to asimialtors and auxotrophs, but is not supre abundant between them
tmpB %>% filter(ind.vari == "DHPS.Degradation") # fidelity
tmpA %>% filter(ind.vari == "DHPS.Degradation") # specificity
temp %>% select(DHPS.Degradation, taxa_elm, Lifestyle) %>% filter(DHPS.Degradation == 1) %>% group_by(Lifestyle) %>% mutate(sum = n()) %>% group_by(taxa_elm, Lifestyle) %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
temp %>% select(DHPS.Degradation, taxa_elm, Lifestyle) %>% filter(DHPS.Degradation == 1) %>% mutate(sum = n()) %>% filter(taxa_elm %in% c("Alpha-Pelagibacter", "Alpha-Rhodobacter", "Alpha-Other", "Alpha-Rhodobacter-LowGC", "Alpha-Puniceispirilla"))  %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary))  %>% group_by(Lifestyle)
# determine how many alpha proteos have it
temp %>% select(DHPS.Degradation, taxa_elm) %>% filter(taxa_elm %in% c("Alpha-Pelagibacter", "Alpha-Rhodobacter", "Alpha-Other", "Alpha-Rhodobacter-LowGC", "Alpha-Puniceispirilla")) %>% mutate(sum = n()) %>% group_by(DHPS.Degradation) %>% mutate(perc = n()/sum)

# choline o sulfate degradation
tmpAB %>% filter(ind.vari == "Choline.O.sulfate.Degradation") 
temp %>% select(Choline.O.sulfate.Degradation, taxa_elm, Lifestyle) %>% filter(Choline.O.sulfate.Degradation == 1) %>% group_by(Lifestyle) %>% mutate(sum = n()) %>% group_by(taxa_elm, Lifestyle) %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
temp %>% select(Choline.O.sulfate.Degradation, taxa_elm, Lifestyle) %>% filter(Choline.O.sulfate.Degradation == 1) %>% mutate(sum = n()) %>% group_by(taxa_elm, Lifestyle) %>% mutate(n = n()) %>% distinct() %>% mutate(summary = n/sum) %>% arrange(., desc(summary))
temp %>% select(Choline.O.sulfate.Degradation, taxa_elm) %>% filter(Choline.O.sulfate.Degradation == 1) %>% mutate(sum = n()) %>% group_by(taxa_elm) %>% mutate(n = n()) %>% distinct() %>% mutate(summary = n/sum) %>% arrange(., desc(summary))
temp %>% select(Choline.O.sulfate.Degradation, taxa_elm, Lifestyle) %>% filter(Choline.O.sulfate.Degradation == 1 & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(taxa_elm) %>% mutate(n = n()) %>% distinct() %>% mutate(summary = n/sum) %>% arrange(., desc(summary))
temp %>% select(Choline.O.sulfate.Degradation, taxa_elm, Lifestyle) %>% filter(Choline.O.sulfate.Degradation == 1 & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% filter(taxa_elm %in% c("Alpha-Pelagibacter", "Alpha-Rhodobacter", "Alpha-Other", "Alpha-Rhodobacter-LowGC", "Alpha-Puniceispirilla"))  %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary))

# taurine degradation
tmpAB %>% filter(ind.vari == "Taurine.Degradation") 
temp %>% select(Taurine.Degradation, taxa_elm, Lifestyle) %>% filter(Taurine.Degradation == 1) %>% group_by(Lifestyle) %>% mutate(sum = n()) %>% group_by(taxa_elm, Lifestyle) %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
temp %>% select(Taurine.Degradation, taxa_elm, Lifestyle) %>% filter(Taurine.Degradation == 1) %>% mutate(sum = n()) %>% group_by(taxa_elm, Lifestyle) %>% mutate(n = n()) %>% distinct() %>% mutate(summary = n/sum) %>% arrange(., desc(summary))
temp %>% select(Taurine.Degradation, taxa_elm) %>% filter(Taurine.Degradation == 1) %>% mutate(sum = n()) %>% group_by(taxa_elm) %>% mutate(n = n()) %>% distinct() %>% mutate(summary = n/sum) %>% arrange(., desc(summary))
temp %>% select(Taurine.Degradation, taxa_elm, Lifestyle) %>% filter(Taurine.Degradation == 1 & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(taxa_elm) %>% mutate(n = n()) %>% distinct() %>% mutate(summary = n/sum) %>% arrange(., desc(summary))
temp %>% select(Taurine.Degradation, taxa_elm, Lifestyle) %>% filter(Taurine.Degradation == 1 & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% filter(taxa_elm %in% c("Alpha-Pelagibacter", "Alpha-Rhodobacter", "Alpha-Other", "Alpha-Rhodobacter-LowGC", "Alpha-Puniceispirilla"))  %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary))


# glutathione disulfide synthesis and degradation
temp%>% select(Glutathione.disulfide.degradation.and.synthesis, taxa_elm, Lifestyle) %>% filter(taxa_elm %in% c( "Alpha-Rhodobacter", "Alpha-Other", "Alpha-Rhodobacter-LowGC", "Alpha-Puniceispirilla") & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.disulfide.degradation.and.synthesis) %>% mutate(perc = n()/sum)
temp%>% select(Glutathione.disulfide.degradation.and.synthesis, taxa_elm, Lifestyle) %>% filter(taxa_elm %in% c( "Gamma-SAR92", "Gamma-Other", "Gamma-Alteromonadaceae")& Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.disulfide.degradation.and.synthesis) %>% mutate(perc = n()/sum)
temp%>% select(Glutathione.disulfide.degradation.and.synthesis, taxa_elm, Lifestyle) %>% filter(taxa_elm %in% c( "Cyano-Pro", "Cyano-Syn", "Cyano-Other")& Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.disulfide.degradation.and.synthesis) %>% mutate(perc = n()/sum)
tmpAB %>% filter(ind.vari == "Glutathione.disulfide.degradation.and.synthesis")
temp %>% select(Glutathione.disulfide.degradation.and.synthesis, taxa_elm, Lifestyle) %>% filter(Glutathione.disulfide.degradation.and.synthesis == 1) %>% group_by(Lifestyle) %>% mutate(sum = n()) %>% group_by(taxa_elm, Lifestyle) %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
temp %>% select(Glutathione.disulfide.degradation.and.synthesis, taxa_elm, Lifestyle) %>% filter(Glutathione.disulfide.degradation.and.synthesis == 1, Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(taxa_elm) %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
0.433346441 + 0.207235549 + 0.155721589 + 0.086118757 + 0.010617381

# glutathione synthesis
tmpAB %>% filter(ind.vari == "Glutathione.Synthesis")
temp%>% select(Glutathione.Synthesis, taxa_elm, Lifestyle) %>% filter(taxa_elm %in% c( "Alpha-Rhodobacter", "Alpha-Other", "Alpha-Rhodobacter-LowGC", "Alpha-Puniceispirilla") & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.Synthesis) %>% mutate(perc = n()/sum)
0.054766734 + 0.944219067
temp%>% select(Glutathione.Synthesis, taxa_elm, Lifestyle) %>% filter(taxa_elm %in% c( "Gamma-SAR92", "Gamma-Other", "Gamma-Alteromonadaceae") & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.Synthesis) %>% mutate(perc = n()/sum)
0.898589657 + 0.098052384
temp%>% select(Glutathione.Synthesis, taxa_elm, Lifestyle) %>% filter(taxa_elm %in% c("Cyano-Pro", "Cyano-Syn", "Cyano-Other") & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.Synthesis) %>% mutate(perc = n()/sum)
0.76353276 + 0.17663818
tmpAB %>% filter(ind.vari == "Glutathione.Synthesis")
temp %>% select(Glutathione.Synthesis, Lifestyle)%>% filter(Glutathione.Synthesis != 0 & Lifestyle == "assimilator") %>% mutate(sum = n()) %>% group_by(Glutathione.Synthesis)  %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
temp %>% select(Glutathione.Synthesis, Lifestyle, taxa_elm)%>% filter(Glutathione.Synthesis != 0 & Lifestyle == "assimilator" & taxa_elm %in% c("Cyano-Pro", "Cyano-Syn", "Cyano-Other")) %>% mutate(sum = n()) %>% group_by(Glutathione.Synthesis)  %>% mutate(n= n()) %>% mutate(summary = n/sum) %>% distinct() %>% arrange(., desc(summary)) 
tmpAB %>% filter(key == "score_assimilator") %>% arrange(., desc(value))

# methionine salvage
test_df <- read.csv("kofam_results/Methionine Salvage Part 1processed_above75genomecompletion.csv")
test_df <- test_df %>% filter(X %in% names_id_filtered$Genome)
test_df %>% group_by(MaxCompletion) %>% summarise(n = n()) %>% mutate(sum = sum(n)) %>% mutate(perc = n/sum)

test_df <- read.csv("kofam_results/Methionine Salvage Part 2processed_above75genomecompletion.csv")
test_df <- test_df %>% filter(X %in% names_id_filtered$Genome)
test_df %>% group_by(MaxCompletion) %>% summarise(n = n()) %>% mutate(sum = sum(n)) %>% mutate(perc = n/sum)
```
...
taxa <- read.csv("/Users/michelle/Documents/DOSTraits/new_filtered_SA/2025_1022_MAD_SulfateAssimilationprocessed_above75genomecompletion.csv")[,-1]
head(taxa)
...
#remove any genomes not assigned a GTDB taxonomy, if this hasn't been done already
# idx <- which(is.na(taxa$Phyla))
# length(idx)
# abund <- abund[-idx,]
# df <- df[-idx,]
# taxa <- taxa[-idx,]
#remove any MAGs with only zeros, if this hasn't been done already
...
ggplot(fd_IV, aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 5, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c("#0016a6",'#415521','#A88205')) + theme_bw()+
...
ggplot(fd_IV, aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 5, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c("#0016a6",'#497502','#F5D467')) + theme_bw()+
  ylim(0,1) + theme_linedraw()+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d2.pdf", unit = "in", height = 5, width = 20)   
ggplot(fd_IV, aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(size = 5, alpha = .8, fill = "black") + 
  scale_shape_manual(values = c(21,24))+
  theme_bw()+
  ylim(0,1) + theme_linedraw()+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d3.pdf", unit = "in", height = 5, width = 20)  
```
### Make the supplemental figure that relates to Figure 4
This will use the non presence absence space.
```{r}
rbind(tmpA,tmpB) %>%
  pivot_wider(names_from  = score, values_from = value) %>%
  mutate(score = sqrt(A*B)) %>%
...
tmpAB <- rbind(tmpA,tmpB) %>%
  ...
  arrange(score_diff) %>% print(n=42)
tmpAB <- gather(tmpAB[,c(1,6,7)], key = key, value = value, score_assimilator:score_auxotroph)
write.csv(tmpAB, "2025Results/IndicatorValues_2.csv",quote = F, col.names = T)
write.csv(temp_indicatorgroup, "2025Results/IndicatorGroups_2.csv",quote = F, col.names = T)
...
ggplot(fd_IV, aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  ...
  ylim(0,1) + theme_linedraw()+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d4.pdf", unit = "in", height = 5, width = 20)   
ggplot(fd_IV, aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 5, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c("#0016a6",'#497502','#F5D467')) + theme_bw()+
  ylim(0,1) + theme_linedraw()+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d5.pdf", unit = "in", height = 5, width = 20)   
ggplot(fd_IV, aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(size = 5, alpha = .8, fill = "black") + 
  scale_shape_manual(values = c(21,24))+
  theme_bw()+
  ylim(0,1) + theme_linedraw()+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d6.pdf", unit = "in", height = 5, width = 20)  

fd_IV %>% filter(indicator.group == "common") %>% ggplot(aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 5, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c('#A88205')) + theme_bw()+
  ylim(0,1) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d_commoninds.pdf", unit = "in", height = 5, width = (length(common_paths)+2))   
fd_IV %>% filter(indicator.group == "auxotroph") %>% ggplot(aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 5, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c('#497502')) + theme_bw()+
  ylim(0,1) +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d_auxinds.pdf", unit = "in", height = 5, width = length(aux_paths) +2) 
fd_IV %>% filter(indicator.group == "assimilator") %>% ggplot(aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 5, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c('#0016a6')) + theme_bw()+
  ylim(0,1) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d_assiminds.pdf", unit = "in", height = 5, width = length(assim_paths) +2) 

fd_IV %>% filter(indicator.group == "common") %>% ggplot(aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 7, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c('#A88205')) + theme_bw()+
  ylim(0,1) +theme(axis.text.x = element_blank()) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d_commoninds_blank.pdf", unit = "in", height = 5, width = (length(common_paths)+2))   
fd_IV %>% filter(indicator.group == "auxotroph") %>% ggplot(aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size =7, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c('#497502')) + theme_bw()+
  ylim(0,1) +theme(axis.text.x = element_blank())+
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d_auxinds_blank.pdf", unit = "in", height = 5, width = length(aux_paths) +2) 
fd_IV %>% filter(indicator.group == "assimilator") %>% ggplot(aes(x = factor(ind.vari, order), y = value, shape = key)) + 
  geom_point(aes(fill = indicator.group), size = 7, alpha = .8, color = "black") + scale_shape_manual(values = c(21,24))+
  scale_fill_manual(values = c('#0016a6')) + theme_bw()+
  ylim(0,1)  +theme(axis.text.x = element_blank()) +
  theme(plot.margin = unit(c(0,0,0,2), "cm")) 
ggsave(plot = last_plot(), file = "2025Results/fig4Varations/fig4_d_assiminds_blank.pdf", unit = "in", height = 5, width = 21) 
...
fasta_16s_file <- seqinr::read.fasta(file = "go_microbiomics-integrated-cpl50_ctn10-16S.fasta", seqtype = "DNA",as.string = TRUE, set.attributes = FALSE)
names(fasta_16s_file) <- gsub(names(fasta_16s_file), pattern = "gene_.*", replacement = "")
# load the scaffold membership file
table <- read.table("genomes-scaffolds-membership.tsv", sep = "\t")
...
#FALSE
fasta_16s_file <- fasta_16s_file[which(names(fasta_16s_file) %in% our_genomes$Genome)]
write.fasta(sequences = fasta_16s_file, names = names(fasta_16s_file), nbchar = 80, file.out = "go_microbiomics-16S_11032025.fasta")
...
tree1 <- read.tree("2025Results/fig5Variations/16s_newick_tree_4")
p7 <- ggtree(tree1, layout = "fan", open.angle=15, size =0.1) + geom_tiplab(size = 0)
...
write.csv(df3, "2025Results/fig5Variations/fig5_annotation_pathways.csv", quote = F, row.names = T)
write.csv(df4, "2025Results/fig5Variations/fig5_annotation_lifestyles.csv", quote = F, row.names = T)
write.csv(df5, "2025Results/fig5Variations/fig5_annotation_taxa.csv", quote = F, row.names = T)
write.csv(df6, "2025Results/fig5Variations/fig5_annotation_genomecompletion.csv", quote = F, row.names = T)
...
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_a.pdf", unit = "in", height = 30, width = 30) 
...
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_b.pdf", unit = "in", height = 30, width = 30) 
gheatmap(p7, df5, offset=0.2, width=0.6, 
         colnames=TRUE, legend_title="Taxa", colnames_angle = 90, colnames_offset_y = 40)
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_c.pdf", unit = "in", height = 30, width = 30) 
...
gheatmap(p3, df5, offset=5, width=0.6, 
         colnames=TRUE, legend_title="Taxa", colnames_angle = 90, colnames_offset_y = 40)
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_d.pdf", unit = "in", height = 40, width = 40) 
...
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_e.pdf", unit = "in", height = 40, width = 40) 
...
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_f.pdf", unit = "in", height = 40, width = 40) 
gheatmap(p4, df3, offset=.53, width=0.82, 
         colnames=TRUE,colnames_offset_x = 0, color = NA,
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "#A88205")
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_f_ochre.pdf", unit = "in", height = 40, width = 40) 
gheatmap(p4, df3, offset=.53, width=0.82, 
         colnames=TRUE,colnames_offset_x = 0, color = NA,
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "#2F397A")
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_f_blue.pdf", unit = "in", height = 40, width = 40) 
gheatmap(p4, df3, offset=.53, width=0.82, 
         colnames=TRUE,colnames_offset_x = 0, color = NA,
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "#415521")
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_f_green.pdf", unit = "in", height = 40, width = 40) 
...
gheatmap(p4, df3, offset=.53, width=0.82, 
         ...
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "black")  +scale_x_continuous(limits=c(-1, NA))
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_g.pdf", unit = "in", height = 40, width = 50, limitsize = FALSE) 
gheatmap(p4, df3, offset=.53, width=0.82, 
         colnames=TRUE,colnames_offset_x = 0, color = NA,
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "#A88205") +scale_x_continuous(limits=c(-1, NA))
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_g_ochre.pdf", unit = "in", height = 40, width = 50, limitsize = FALSE) 
gheatmap(p4, df3, offset=.53, width=0.82, 
         colnames=TRUE,colnames_offset_x = 0, color = NA,
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "#2F397A")  +scale_x_continuous(limits=c(-1, NA))
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_g_blue.pdf", unit = "in", height = 40, width = 50, limitsize = FALSE) 
gheatmap(p4, df3, offset=.53, width=0.82, 
         colnames=TRUE,colnames_offset_x = 0, color = NA,
         hjust = 1, legend_title="Present or Absent", colnames_angle = 90, 
         colnames_offset_y = 0) + 
  scale_fill_gradient(low = "white", high = "#415521") +scale_x_continuous(limits=c(-1, NA))
ggsave(plot = last_plot(), file = "2025Results/fig5Variations/fig5_g_green.pdf", unit = "in", height = 40, width = 50, limitsize = FALSE) 
...
# filter to the genomes in our dataset
MS <- MS %>% filter(Row.names %in% names_id_filtered$Genome)
# add correct taxa
MS <- merge(MS, names_id_filtered, by.x = "Row.names", by.y = "Genome")
MS$Group <- NULL
...
# answer questions about the first half
MS %>% select(`MSP1:Step1`,`MSP1:Step2`,`MSP1:Step3`,`MSP1:Step4`,`MSP1:Step5`,`MSP1:Step6`,`MSP1:Step7`,taxa_elm,Type,Completion) %>% summarise()
#In the first half, are they using the shunt or the longer pathway?
MS %>% select(`MSP1:Step1`,`MSP1:Step2`,`MSP1:Step3`,`MSP1:Step4`,`MSP1:Step5`,`MSP1:Step6`,`MSP1:Step7`,taxa_elm,Type,Completion) %>% mutate(long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% group_by(long, shunt) %>% filter(long == 1 | shunt == 1) %>% summarise(n=n()) %>% arrange(., desc(n))

#Which genomes can bring it to MTA and no further?
MS %>% select(`MSP1:Step1`,`MSP1:Step2`,`MSP1:Step3`,`MSP1:Step4`,`MSP1:Step5`,`MSP1:Step6`,`MSP1:Step7`,taxa_elm,Type,Completion) %>% mutate(long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% filter(`MSP1:Step1` == 1 & `MSP1:Step2` == 1 & `MSP1:Step3` == 1 & (`MSP1:Step7` == 0 | `MSP1:Step6` == 0 | `MSP1:Step4` == 0 &`MSP1:Step5`==0))
MS %>% select(`MSP1:Step1`,`MSP1:Step2`,`MSP1:Step3`,`MSP1:Step4`,`MSP1:Step5`,`MSP1:Step6`,`MSP1:Step7`,taxa_elm,Type,Completion) %>% mutate(long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% filter(`MSP1:Step1` == 1 & `MSP1:Step2` == 1 & `MSP1:Step3` == 1 & (`MSP1:Step7` == 0 | `MSP1:Step6` == 0 | `MSP1:Step4` == 0 &`MSP1:Step5`==0) & long !=1 & shunt != 1) 

# answer questions about the second half
MS %>% select(`MSP2:Step1`,`MSP2:Step2`, `MSP2:Step3`, `MSP2:Step4`, `MSP2:Step5`, `MSP2:Step6`, `MSFE:Step1`, `MSFE:Step2`, `MSIS:Step1`, `MSIS:Step2`, `MSIS:Step3`, `MSAD:Step1`,taxa_elm,Type,Completion) 
#which genomes have a canonical second half?
MS %>% select(`MSP2:Step1`,`MSP2:Step2`, `MSP2:Step3`, `MSP2:Step4`, `MSP2:Step5`, `MSP2:Step6`, `MSFE:Step1`, `MSFE:Step2`, `MSIS:Step1`, `MSIS:Step2`, `MSIS:Step3`, `MSAD:Step1`,taxa_elm,Type,Completion) %>% mutate(can = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(can_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% filter(can == 1 | can_long == 1) %>% summarise(n= n())
#Which genomes have a second half? 
MS %>% select(`MSP2:Step1`,`MSP2:Step2`, `MSP2:Step3`, `MSP2:Step4`, `MSP2:Step5`, `MSP2:Step6`, `MSFE:Step1`, `MSFE:Step2`, `MSIS:Step1`, `MSIS:Step2`, `MSIS:Step3`, `MSAD:Step1`,taxa_elm,Type,Completion) %>% mutate(can = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(can_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% filter(can == 1 | can_long == 1 | IS == 1 | FE == 1 | AD ==1) %>% summarise(n= n())
MS %>% select(`MSP2:Step1`,`MSP2:Step2`, `MSP2:Step3`, `MSP2:Step4`, `MSP2:Step5`, `MSP2:Step6`, `MSFE:Step1`, `MSFE:Step2`, `MSIS:Step1`, `MSIS:Step2`, `MSIS:Step3`, `MSAD:Step1`,taxa_elm,Type,Completion) %>% mutate(can = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(can_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% filter(FE == 1)
MS %>% select(`MSP2:Step1`,`MSP2:Step2`, `MSP2:Step3`, `MSP2:Step4`, `MSP2:Step5`, `MSP2:Step6`, `MSFE:Step1`, `MSFE:Step2`, `MSIS:Step1`, `MSIS:Step2`, `MSIS:Step3`, `MSAD:Step1`,taxa_elm,Type,Completion) %>% mutate(can = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(can_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% filter( IS == 1 ) %>% summarise(n= n())

#Which genomes overlap and have both a first and second half? 
#canonical
MS %>% mutate(MSP1_long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(MSP1_shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% mutate(MSP2_shunt = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(MSP2_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% 
  mutate(v1 = MSP1_long + MSP2_long) %>% 
  mutate(v2 = MSP1_long + MSP2_shunt) %>%   
  mutate(v3 = MSP1_long + IS) %>%  
  mutate(v4 = MSP1_long + FE) %>% 
  mutate(v5 = MSP1_long + AD) %>% 
  mutate(v6 = MSP1_shunt + MSP2_long) %>%  
  mutate(v7 = MSP1_shunt + MSP2_shunt) %>%  
  mutate(v8 = MSP1_shunt + IS) %>%  
  mutate(v9 = MSP1_shunt + FE) %>% 
  mutate(v10 = MSP1_shunt + AD) %>% 
  filter(v1 == 2 | v2 == 2 | v6 == 2 | v7 == 2 )
#isoprenoid shunt 
MS %>% mutate(MSP1_long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(MSP1_shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% mutate(MSP2_shunt = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(MSP2_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% filter(IS == 1) %>% group_by(taxa_elm) %>% summarise(n = n())
#isoprenoid shunt completion:
MS %>% mutate(MSP1_long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(MSP1_shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% mutate(MSP2_shunt = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(MSP2_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% 
  mutate(v1 = MSP1_long + MSP2_long) %>% 
  mutate(v2 = MSP1_long + MSP2_shunt) %>%   
  mutate(v3 = MSP1_long + IS) %>%  
...
MS %>% mutate(MSP1_long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(MSP1_shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% mutate(MSP2_shunt = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(MSP2_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% 
  ...
  mutate(v9 = MSP1_shunt + FE) %>% 
  mutate(v10 = MSP1_shunt + AD) %>% 
  filter(AD == 1) %>% group_by(taxa_elm) %>% summarise(n= n())
MS %>% mutate(MSP1_long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(MSP1_shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% mutate(MSP2_shunt = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(MSP2_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% 
  mutate(v1 = MSP1_long + MSP2_long) %>% 
  mutate(v2 = MSP1_long + MSP2_shunt) %>%   
  mutate(v3 = MSP1_long + IS) %>%  
...
MS %>% mutate(MSP1_long = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step4`+`MSP1:Step5`+`MSP1:Step7`)/6) %>% mutate(MSP1_shunt = (`MSP1:Step1`+`MSP1:Step2`+`MSP1:Step3`+`MSP1:Step6`+`MSP1:Step7`)/5) %>% mutate(MSP2_shunt = (`MSP2:Step1`+ `MSP2:Step4`+`MSP2:Step5`+`MSP2:Step6`)/4) %>% mutate(MSP2_long = (`MSP2:Step1`+`MSP2:Step2`+ `MSP2:Step3`+ `MSP2:Step5`+`MSP2:Step6`)/5) %>% mutate(IS = (`MSIS:Step1`+ `MSIS:Step2`+`MSIS:Step3`)/3) %>% mutate(FE = (`MSFE:Step1`+ `MSFE:Step2`)/2) %>% mutate(AD = `MSAD:Step1`) %>% 
  ...
  mutate(v9 = MSP1_shunt + FE) %>% 
  mutate(v10 = MSP1_shunt + AD) %>% 
  filter(v1 == 2 | v2 == 2 | v3 == 2 | v4 == 2 | v5 == 2 | v6 ==2 | v7 ==2 | v8 == 2 | v9 == 2| v10 == 2)
```
